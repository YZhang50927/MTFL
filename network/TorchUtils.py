...
class TorchClassModel(TorchModel):

    def __init__(self, model, classifier):
        super(TorchClassModel, self).__init__(model)
    ...
    @classmethod
    def load_model_classifier(cls, model_path, classifier_path):
        """
        Loads a pickled model
        Args:
            model_path: path to the pickled model
            classifier_path: path to the pickled classifier model

        Returns: TorchModel class instance wrapping the provided model
        """
        return cls(load_model(model_path), load_model(classifier_path))
    ...
    def do_epoch(self, criterion, optimizer, data_iter, epoch):
        ...
        for iteration, (batch, targets) in enumerate(data_iter):
            ...
            outputs = self.model(batch).cpu().detach().numpy()

            for output, feature, target in zip(outputs, batch, targets):
                mask = (output > self.threshold).flatten()
                if sum(mask) == 0:
                    continue

                pred_class = self.classifier(feature[mask]).cpu()
                predictions = torch.cat((predictions, pred_class), 0)
                class_targets = torch.cat((class_targets, target.repeat(sum(mask), 1).cpu()), 0)
            # if iteration % 100 == 0:
            #     print(predictions[0])
            #     print(class_targets[0])
            #     print(predictions[-1])
            #     print(class_targets[-1])
            loss = criterion(predictions, class_targets)
            # Backward and optimize
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    def save(self, model_path):
        ...
        """
        if self.is_data_parallel:
            torch.save(self.classifier.module, model_path)
        else:
