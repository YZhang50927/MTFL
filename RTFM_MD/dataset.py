...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            feature_subpath, cls_num, start_end_couples, num_frames = self.list[index]
            features = read_features(feature_subpath, model=self.type)
            label = torch.tensor(cls_num)
            if self.transform is not None:
                features = self.transform(features)
            return torch.unsqueeze(features, 0), label, start_end_couples, num_frames  #[N,B,T,F]
        else:
            feature_subpath, cls_num = self.list[index]
            features = read_features(feature_subpath, model=self.type)
            label = torch.tensor(cls_num)
            if self.transform is not None:
                features = self.transform(features)
        ...
        with open(annotation_path) as f:
            ...
            for line in lines:
                ...
                if self.test_mode:
                    ...
                    features_list.append((feature_path, cls_num, start_end_couples, num_frames))
                elif (cls_num == class_to_int['Normal']) == self.is_normal:
                    features_list.append((feature_path, cls_num))
