...
class Model(nn.Module):
    ...
    def forward(self, inputs):
        ...
        features = self.Aggregate(out) # B T D

        out = torch.mean(features, dim=1) # B D
        out = self.drop_out(out)
        scores = self.relu(self.fc1(out))
        scores = self.drop_out(scores)
        scores = self.relu(self.fc2(scores))
        scores = self.drop_out(scores)
        scores = self.sigmoid(self.fc3(scores)) # B 18
        ...
        return feat_select_abn, feat_select_normal, feat_select_abn, feat_select_abn, scores, feat_select_abn, feat_select_abn, feat_magnitudes