...
class DefaultModelCallback(Callback):
    ...
    def on_training_end(self, model):
        if self.visualization_dir is not None:
            ...
            plt.close()
            with open(os.path.join(self.visualization_dir, 'loss.txt'), 'w') as f:
                f.writelines([str(loss)+'\n' for loss in self.train_losses])
            with open(os.path.join(self.visualization_dir, 'fine_loss.txt'), 'w') as f:
                f.writelines([str(loss)+'\n' for loss in self.fine_losses])
