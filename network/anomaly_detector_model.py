...
def custom_objective_mean_normal(y_pred, y_true):
    # y_pred (batch_size, 32, 1)
    # y_true (batch_size)
    lambdas = 8e-5
    ...
    # get the max score for each video
    normal_segments_scores_means = normal_segments_scores.mean(dim=-1)[0]
    anomal_segments_scores_maxes = anomal_segments_scores.max(dim=-1)[0]

    hinge_loss = 1 - anomal_segments_scores_maxes + normal_segments_scores_means
    hinge_loss = torch.max(hinge_loss, torch.zeros_like(hinge_loss))
