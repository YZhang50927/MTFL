...
def train(nloader, aloader, model, batch_size, optimizer, viz, device):
    with torch.set_grad_enabled(True):
        ...
        scores = scores.view(batch_size * 32 * 2, -1) #2B * T, 18

        abn_scores, indice = torch.max(scores[batch_size*32:], dim=1)
        nor_scores, indice = torch.max(scores[0:batch_size*32], dim=1)

        nlabel = nlabel[0:batch_size]
        alabel = alabel[0:batch_size]

        loss_criterion = RTFM_loss(0.0001, 100)
        loss_sparse = sparsity(abn_scores, batch_size, 8e-3) # 时间域上异常稀疏
        loss_smooth = smooth(abn_scores, 8e-4) #异常分数平滑
