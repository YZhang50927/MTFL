...
if __name__ == '__main__':
    ...
    model_path = os.path.join(args.save_models, args.experiment)
    ...
    scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=400, gamma=1)

    test_info = {"epoch": [], "UCF_AUC": [], "VAD3_AUC": []}
    best_VAD3_AUC = -1
    best_UCF_AUC = -1
    output_path = args.output_path  # put your own path here
    _, _, auc_UCF = test(test_loader_UCF, model, args.seg_num, 'UCF', viz, device)
    #_, _, auc_VAD3 = test(test_loader_VAD3, model, args.seg_num, 'VAD3', viz, device)
    ...
    for step in tqdm(
        ...
        train(loadern_iter, loadera_iter, model, args.batch_size, args.seg_num, optimizer, scheduler, viz, device)

        if step % 5 == 0 and step > 200:

            fpr_UCF, tpr_UCF, auc_UCF = test(test_loader_UCF, model, args.seg_num, 'UCF', viz, device)
            fpr_VAD3, tpr_VAD3, auc_VAD3 = test(test_loader_VAD3, model, args.seg_num, 'VAD3', viz, device)
            ...
            if test_info["UCF_AUC"][-1] > best_UCF_AUC or test_info["VAD3_AUC"][-1] > best_VAD3_AUC:
                torch.save(model.state_dict(),
                           os.path.join(model_path,
                                        args.model_name + '-' + args.experiment + '-{}.pkl'.format(step)))
                if test_info["UCF_AUC"][-1] > best_UCF_AUC:
                    best_UCF_AUC = test_info["UCF_AUC"][-1]
                    save_path = os.path.join(output_path, args.experiment, 'UCF')
                ...
                if test_info["VAD3_AUC"][-1] > best_VAD3_AUC:
                    ...
                    save_path = os.path.join(output_path, args.experiment, 'VAD3')
                    if not os.path.exists(save_path):
                        os.makedirs(save_path)
                    save_best_record(test_info, os.path.join(save_path, '{}-step-AUC.txt'.format(step)))
    ...
    torch.save(model.state_dict(), os.path.join(model_path, args.model_name + '-' + args.experiment + 'final.pkl'))
