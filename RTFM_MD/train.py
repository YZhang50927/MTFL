...
class RTFM_loss(torch.nn.Module):
    def __init__(self, alpha, margin):
        ...
        self.criterion = torch.nn.BCELoss() # multi class

    def forward(self, score_normal, score_abnormal, nlabel, alabel, feat_n, feat_a):
        ori_label = torch.cat((nlabel, alabel), 0)
        label = F.one_hot(ori_label, num_classes=18)
        label = label.float()
        ...
        loss_cls = self.criterion(score, label)  # CE loss in the score space
