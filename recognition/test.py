...
def test(dataloader, model, device, class_criteria='UCF'):
    ...
    with torch.no_grad():
        ...
        if class_criteria == 'UCF': # all road accidents in UCF are labelled as 13
            for row in outputs:
        ...
        print('\n Evaluation based on the classes in' + str(class_criteria) + ' top1 : ' + str(res[0]) +
              ' top5 : ' + str(res[1]) + '\n')
...
def main():
    ...
    video_class = test(dataloader=test_loader,
                       model=model,
                       device=device,
                       test_acc=False,
                       test_dataset='VAD')
